    ...
    public class DocumentProcessor
    {
        ...
        public async Task ProcessAsync(DocumentIngestionRequest request, IProgress<(int completed, int total)> progress = null)
        {
            ...
            for (int b = 0; b < batches.Count; b++)
            {
                ...
                _logger?.LogInformation("Processor: Embedding batch {BatchIndex} size={Count} took {Elapsed}ms",
                ...
                _logger?.LogInformation("Processor: Upsert batch {BatchIndex} points={Count} took {Elapsed}ms",
                    b, items.Count, swUpsert.ElapsedMilliseconds);
